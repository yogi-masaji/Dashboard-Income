...
<body class="preloader-active">
    ...
    <style>
        ...
        .btn-submit:hover {
            background-color: #d99e02; /* Warna hover diubah menjadi kuning lebih gelap */
            transform: translateY(-2px);
            box-shadow: 0 5px 15px rgba(217, 158, 2, 0.3);
        }
    ...
    {{-- START: CSS BARU UNTUK NAVIGASI AKTIF --}}
    <style>
        /* Class baru untuk item navigasi yang aktif */
        .nav-item-active > .menu-link {
            background: #fcb900 !important; /* Latar belakang kuning cerah */
            color: #333 !important; /* Teks gelap untuk kontras */
            border-radius: 0.375rem; /* Sudut membulat */
            margin: 0.125rem 0.5rem; /* Sedikit spasi vertikal dan horizontal */
            box-shadow: 0 4px 8px -2px rgba(252, 185, 0, 0.4); /* Bayangan halus */
        }

        /* Pastikan ikon dan teks di dalam link aktif memiliki warna yang benar */
        .nav-item-active > .menu-link i,
        .nav-item-active > .menu-link .text-truncate {
            color: #333 !important;
        }

        /* Gaya untuk induk dari item submenu yang aktif */
        .menu-item.open > .menu-toggle {
            background-color: rgba(255, 255, 255, 0.05) !important; /* Sedikit sorot induknya */
        }
        .dark-mode .bg-menu-theme .menu-item > .menu-link:not(.menu-toggle):hover {
    background-color: #b59f63 !important;
}
    </style>
    {{-- END: CSS BARU UNTUK NAVIGASI AKTIF --}}
        ...
        <div class="layout-container">
            ...
            <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
                ...
                <div class="scrollbar-container"
                    ...
                    <ul class="menu-inner py-1">
                        @foreach ($navbarMenus as $menuItem)
                            @if ($menuItem['children']->isNotEmpty())
                                @php
                                    // Buat array dari route anak untuk diperiksa dengan request saat ini
                                    $childRoutes = $menuItem['children']->map(function ($child) {
                                        return ltrim($child->nama_File ?? '', '/');
                                    })->filter()->all();

                                    // Periksa apakah URL saat ini cocok dengan salah satu route anak
                                    $isParentActive = request()->is($childRoutes);
                                @endphp
                                {{-- Jika anak aktif, tambahkan class 'open' ke li induk --}}
                                <li class="menu-item {{ $isParentActive ? 'open' : '' }}">
                                    ...
                                    <ul class="menu-sub">
                                        @foreach ($menuItem['children'] as $submenu)
                                            @php
                                                $submenuUrl = ltrim($submenu->nama_File ?? '', '/');
                                            @endphp
                                            {{-- Tambahkan class 'nav-item-active' ke item submenu yang aktif --}}
                                            <li class="menu-item {{ request()->is($submenuUrl) ? 'nav-item-active' : '' }}">
                            ...
                            @else
                                @php
                                    $menuUrl = ltrim($menuItem['url'] ?? '', '/');
                                @endphp
                                {{-- Tambahkan class 'nav-item-active' ke item menu tunggal yang aktif --}}
                                <li class="menu-item {{ request()->is($menuUrl) ? 'nav-item-active' : '' }}">
                    ...
                    {{-- END: LOGIKA BARU UNTUK NAVIGASI --}}
