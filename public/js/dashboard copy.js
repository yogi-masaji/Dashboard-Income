$(document).ready(function() {
    ...
    // Cek apakah kita berada di halaman yang tepat sebelum menjalankan skrip
    ...
    function updateChartPreservingLegend(chartInstance, newData) {
        ...
        // Iterate over the new datasets and update the corresponding existing dataset in the chart instance.
        // This is crucial because it modifies the existing dataset object in place,
        // preserving properties like `.hidden` that Chart.js manages when a user clicks the legend.
        // Simply replacing the entire `chartInstance.data.datasets` array would discard the user's selection.
        newData.datasets.forEach((newDataset, index) => {
            ...
            if (existingDataset) {
                ...
                // Also update visual properties that might change
                existingDataset.backgroundColor = newDataset.backgroundColor;
                existingDataset.borderColor = newDataset.borderColor;
            }
        });

        chartInstance.update('none'); // 'none' prevents animation during refresh.
    }
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (dailyQuantityBarChart) {
                    updateChartPreservingLegend(dailyQuantityBarChart, barData);
                } else {
                    const commonOptions = {
                ...
                if (dailyQuantityLineChart) {
                    updateChartPreservingLegend(dailyQuantityLineChart, lineData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if(weeklyQuantityBarChart) {
                    updateChartPreservingLegend(weeklyQuantityBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if(weeklyPassQuantityBarChart) {
                    updateChartPreservingLegend(weeklyPassQuantityBarChart, barPassData);
                } else {
                     const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if(weeklyQuantityLineChart) {
                    updateChartPreservingLegend(weeklyQuantityLineChart, lineData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if(weeklyPassQuantityLineChart) {
                    updateChartPreservingLegend(weeklyPassQuantityLineChart, linePassData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (monthlyQuantityBarChart) {
                    updateChartPreservingLegend(monthlyQuantityBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (monthlyPassQuantityBarChart) {
                    updateChartPreservingLegend(monthlyPassQuantityBarChart, barPassData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (monthlyQuantityLineChart) {
                    updateChartPreservingLegend(monthlyQuantityLineChart, lineData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (monthlyPassQuantityLineChart) {
                    updateChartPreservingLegend(monthlyPassQuantityLineChart, linePassData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (weeklyIncomeBarChart) {
                    updateChartPreservingLegend(weeklyIncomeBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (weeklyIncomeLineChart) {
                    updateChartPreservingLegend(weeklyIncomeLineChart, lineData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (monthlyIncomeBarChart) {
                    updateChartPreservingLegend(monthlyIncomeBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (monthlyIncomeLineChart) {
                    updateChartPreservingLegend(monthlyIncomeLineChart, lineData);
                } else {
                    const lineOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
    ...
    function fetchDailyEPaymentSummary() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'dkijack', 'parkee', 'cash'].map((p, i) => ({ no: i + 1, payment: getPaymentLabel(p), yesterday: formatRupiah(yesterday[`${p}payment`]), today: formatRupiah(today[`${p}payment`]) }));
    ...
    function fetchDailyEPaymentChart() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const datasets = paymentTypes.map((p, i) => ({ label: getPaymentLabel(p), data: todayData.map(item => item[`${p}payment`]), backgroundColor: colors[i], borderColor: colors[i], hidden: i > 0, datalabels: { anchor: 'end', align: 'end' } }));

                const dataChart = { labels, datasets };

                if (dailyEPaymentBarChart) {
                    updateChartPreservingLegend(dailyEPaymentBarChart, dataChart);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (dailyEPaymentLineChart) {
                    updateChartPreservingLegend(dailyEPaymentLineChart, { ...dataChart });
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
    ...
    function fetchWeeklyEpayment() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'dkijack', 'parkee', 'cash'].map((p, i) => ({ no: i + 1, payment: getPaymentLabel(p), last_week: formatRupiah(lastWeek[`${p}payment`]), this_week: formatRupiah(thisWeek[`${p}payment`]) }));
                ...
                const paymentTypes = ['emoney', 'flazz', 'brizzi', 'tapcash', 'dkijack', 'parkee', 'cash', 'all'];
                const colors = ['#0D61E2', '#FFB800', '#FF4D4D', '#00C9A7', '#6f42c1', '#9C27B0', '#795548', '#CCCCCC'];

                const datasets = paymentTypes.map((p, i) => ({ label: getPaymentLabel(p), data: thisWeekChart.map(item => item[`${p}payment`]), backgroundColor: colors[i], borderColor: colors[i], hidden: i > 0, datalabels: { anchor: 'end', align: 'end' } }));
                
                const barData = { labels, datasets };
                if (weeklyEPaymentBarChart) {
                    updateChartPreservingLegend(weeklyEPaymentBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (weeklyEPaymentLineChart) {
                    updateChartPreservingLegend(weeklyEPaymentLineChart, lineData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
    ...
    function fetchMonthlyEpayment() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'dkijack', 'parkee', 'cash'].map((p, i) => ({ no: i + 1, payment: getPaymentLabel(p), last_month: formatRupiah(lastMonth[`${p}payment`]), this_month: formatRupiah(thisMonth[`${p}payment`]) }));
                ...
                const datasets = paymentTypes.map((p, i) => ({ label: getPaymentLabel(p), data: Object.values(thisMonthChart).map(item => item[`${p}payment`]), backgroundColor: colors[i], borderColor: colors[i], hidden: i > 0, datalabels: { anchor: 'end', align: 'end' } }));

                const barData = { labels, datasets };
                if (monthlyEPaymentBarChart) {
                    updateChartPreservingLegend(monthlyEPaymentBarChart, barData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
                ...
                if (monthlyEPaymentLineChart) {
                    updateChartPreservingLegend(monthlyEPaymentLineChart, lineData);
                } else {
                    const commonOptions = {
                        responsive: true, maintainAspectRatio: true,
                        plugins: { 
    ...
    /**
     * [MODIFIED] Hides the global preloader and shows the main content.
     * @param {boolean} isError - Indicates if the loading process failed.
     */
    function finishLoading(isError = false) {
        const preloader = $('#preloader');
        const mainContent = $('#main-content');

        if (isError) {
            const loadingText = preloader.find('.loading-text');
            if (loadingText.length) {
                loadingText.text('Gagal memuat data. Silakan segarkan halaman.');
            }
            // Keep the preloader visible to show the error message.
            // Or hide it after a delay:
            // setTimeout(() => { preloader.addClass('hidden'); }, 2000);
        ...
        if (mainContent.length) {
            // The CSS transition for the preloader is 0.7s.
            // We wait for it to finish before showing the content.
            setTimeout(() => {
                mainContent.fadeIn(500); // Use fadeIn for a smooth appearance
