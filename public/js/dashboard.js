$(document).ready(function() {
    ...
    function initializeDataTables() {
        ...
        // Transaction Tables (from testasync.js)
        ...
        // E-Payment Tables (from epayment.js)
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                const commonOptions = {
                    ...
                    plugins: { legend: { position: 'top', labels: { color: '#000' } }, datalabels: { backgroundColor: (ctx) => ctx.dataset.backgroundColor, borderRadius: 4, color: 'white', font: { weight: 'bold' }, formatter: formatQuantity, padding: 6, offset: 8 } },
                    scales: { y: { beginAtZero: true, ticks: { precision: 0, color: '#000' }, grace: '10%' }, x: { ticks: { color: '#000' } } }
                };

                if (dailyQuantityBarChart) dailyQuantityBarChart.destroy();
                ...
                if (dailyQuantityLineChart) dailyQuantityLineChart.destroy();
                const ctxLine = document.getElementById('dailyQuantityLine')?.getContext('2d');
                if (ctxLine) dailyQuantityLineChart = new Chart(ctxLine, { type: 'line', data: lineData, options: { ...commonOptions, plugins: { ...commonOptions.plugins, datalabels: { ...commonOptions.plugins.datalabels, padding: 3, offset: 4 }}}, plugins: [ChartDataLabels] });
            }
        });
    }
    ...
    function fetchWeeklyTransactions() {
        return $.ajax({
            ...
            method: 'GET',
            success: function(response) {
                // Logic for weekly casual and pass transactions
                ...
                // Update casual table
                const casualRows = ['car', 'motorbike', 'truck', 'taxi', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_week: formatQuantity(lastWeekCasual[`total_${v}`]),
                    this_week: formatQuantity(thisWeekCasual[`total_${v}`])
                }));
                ...
                // Update pass table
                const passRows = ['car', 'motorbike', 'truck', 'taxi', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_week: formatQuantity(lastWeekPass[`total_${v}`]),
                    this_week: formatQuantity(thisWeekPass[`total_${v}`])
                }));
                ...
                const createDataset = (data, type, hidden) => ({
                    label: type.charAt(0).toUpperCase() + type.slice(1),
                    data: data.map(item => item[`${type}casual`]),
                    backgroundColor: colors[vehicleTypes.indexOf(type)],
                    borderColor: colors[vehicleTypes.indexOf(type)],
                    hidden,
                    datalabels: { anchor: 'end', align: 'end' }
                });
                
                const createPassDataset = (data, type, hidden) => ({
                    label: type.charAt(0).toUpperCase() + type.slice(1),
                    data: data.map(item => item[`${type}pass`]),
                    backgroundColor: colors[vehicleTypes.indexOf(type)],
                    borderColor: colors[vehicleTypes.indexOf(type)],
                    hidden,
                    datalabels: { anchor: 'end', align: 'end' }
                ...
                if(weeklyQuantityBarChart) weeklyQuantityBarChart.destroy();
                ...
                if(weeklyPassQuantityBarChart) weeklyPassQuantityBarChart.destroy();
                ...
                if(weeklyQuantityLineChart) weeklyQuantityLineChart.destroy();
                ...
                if(weeklyPassQuantityLineChart) weeklyPassQuantityLineChart.destroy();
    ...
    function fetchMonthlyTransactions() {
        return $.ajax({
            ...
            method: 'GET',
            success: function(response) {
                // Logic for monthly casual and pass transactions
                ...
                const casualRows = ['car', 'motorbike', 'truck', 'taxi', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_month: formatQuantity(lastMonthCasual[`total_${v}`]),
                    this_month: formatQuantity(thisMonthCasual[`total_${v}`])
                }));
                ...
                const passRows = ['car', 'motorbike', 'truck', 'taxi', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_month: formatQuantity(lastMonthPass[`total_${v}`]),
                    this_month: formatQuantity(thisMonthPass[`total_${v}`])
                }));
                ...
                const createDataset = (data, type, hidden) => ({
                    label: type.charAt(0).toUpperCase() + type.slice(1),
                    data: Object.values(data).map(week => week[`total_${type}`]),
                    backgroundColor: colors[vehicleTypes.indexOf(type)],
                    borderColor: colors[vehicleTypes.indexOf(type)],
                    hidden
                });
                ...
                const commonOptions = {
                    responsive: true, maintainAspectRatio: true,
                    plugins: { legend: { position: 'top', labels: { color: '#000' } }, datalabels: { backgroundColor: (ctx) => ctx.dataset.backgroundColor, borderRadius: 4, color: 'white', font: { weight: 'bold' }, formatter: formatQuantity, padding: 6, offset: 8, anchor: 'end', align: 'end' } },
                    scales: { y: { beginAtZero: true, ticks: { precision: 0, color: '#000' }, grace: '10%' }, x: { ticks: { color: '#000' } } }
                };

                if (monthlyQuantityBarChart) monthlyQuantityBarChart.destroy();
                ...
                if (monthlyPassQuantityBarChart) monthlyPassQuantityBarChart.destroy();
                ...
                if (monthlyQuantityLineChart) monthlyQuantityLineChart.destroy();
                ...
                if (monthlyPassQuantityLineChart) monthlyPassQuantityLineChart.destroy();
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                $('#dailyIncome tfoot').html(`
                    <tr><th colspan="2" style="text-align:left">All Vehicle</th><th>${formatRupiah(yesterday.grandtotal)}</th><th>${formatRupiah(today.grandtotal)}</th></tr>
                    <tr><th colspan="2" style="text-align:left">All Sticker Income</th><th>${formatRupiah(yesterday.stickerincome)}</th><th>${formatRupiah(today.stickerincome)}</th></tr>
                `);
    ...
    function fetchWeeklyIncome() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['car', 'motorbike', 'truck', 'taxi', 'ticket', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_week: formatRupiah(lastWeekIncome[`${v}income`]),
                    this_week: formatRupiah(thisWeekIncome[`${v}income`])
                }));
                weeklyIncomeTable.clear().rows.add(rows).draw();
                
                $('#weeklyIncome tfoot').html(`
                    <tr><th colspan="2" style="text-align:left">All Casual Income</th><th style="font-size:12px;">${formatRupiah(lastWeekIncome.vehicleincome)}</th><th style="font-size:12px;">${formatRupiah(thisWeekIncome.vehicleincome)}</th></tr>
                    <tr><th colspan="2" style="text-align:left">All Sticker Income</th><th style="font-size:12px;">${formatRupiah(lastWeekIncome.stickerincome)}</th><th style="font-size:12px;">${formatRupiah(thisWeekIncome.stickerincome)}</th></tr>
                `);
                ...
                const datasets = vehicleTypes.map((v, i) => ({
                    label: v.charAt(0).toUpperCase() + v.slice(1),
                    data: thisWeekIncomeBar.map(item => item[`${v}income`]),
                ...
                const commonOptions = {
                    responsive: true, maintainAspectRatio: true,
                    plugins: { legend: { position: 'top', labels: { color: '#000' } }, datalabels: { backgroundColor: (ctx) => ctx.dataset.backgroundColor, borderRadius: 4, color: 'white', font: { weight: 'bold' }, formatter: (val) => formatRupiah(val), padding: 6, offset: 8 } },
                    scales: { y: { beginAtZero: true, ticks: { precision: 0, color: '#000' }, grace: '10%' }, x: { ticks: { color: '#000' } } }
                };

                if (weeklyIncomeBarChart) weeklyIncomeBarChart.destroy();
                ...
                if (weeklyIncomeLineChart) weeklyIncomeLineChart.destroy();
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['car', 'motorbike', 'truck', 'taxi', 'ticket', 'other'].map((v, i) => ({
                    no: i + 1,
                    vehicle: v.charAt(0).toUpperCase() + v.slice(1),
                    last_month: formatRupiah(lastMonthIncome[`${v}income`]),
                    this_month: formatRupiah(thisMonthIncome[`${v}income`])
                }));
                monthlyIncomeTable.clear().rows.add(rows).draw();

                $('#monthlyIncome tfoot').html(`
                    <tr><th colspan="2" style="text-align:left">All Casual Income</th><th style="font-size:12px;">${formatRupiah(lastMonthIncome.vehicleincome)}</th><th style="font-size:12px;">${formatRupiah(thisMonthIncome.vehicleincome)}</th></tr>
                    <tr><th colspan="2" style="text-align:left">All Sticker Income</th><th style="font-size:12px;">${formatRupiah(lastMonthIncome.stickerincome)}</th><th style="font-size:12px;">${formatRupiah(thisMonthIncome.stickerincome)}</th></tr>
                `);
                ...
                const datasets = vehicleTypes.map((v, i) => ({
                    label: v.charAt(0).toUpperCase() + v.slice(1),
                    data: labels.map(label => thisMonthIncomeChart[label][`${v}income`]),
                    backgroundColor: colors[i],
                    borderColor: colors[i],
                    hidden: i > 0
                }));
                ...
                const commonOptions = {
                    responsive: true, maintainAspectRatio: true,
                    plugins: { legend: { position: 'top', labels: { color: '#000' } }, datalabels: { backgroundColor: ctx => ctx.dataset.backgroundColor, borderRadius: 4, color: 'white', font: { weight: 'bold' }, formatter: val => formatRupiah(val), padding: 6, offset: 8, anchor: 'end', align: 'end' } },
                    scales: { y: { beginAtZero: true, ticks: { color: '#000', precision: 0 }, grace: '10%' }, x: { ticks: { color: '#000' } } }
                };

                if (monthlyIncomeBarChart) monthlyIncomeBarChart.destroy();
                ...
                if (monthlyIncomeLineChart) monthlyIncomeLineChart.destroy();
    ...
    function fetchDailyEPaymentSummary() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'parkee', 'cash'].map((p, i) => ({
                    no: i + 1,
                    payment: p.charAt(0).toUpperCase() + p.slice(1).replace('tapcash', 'Tap Cash'),
                    yesterday: formatRupiah(yesterday[`${p}payment`]),
                    today: formatRupiah(today[`${p}payment`])
                }));
    ...
    function fetchDailyEPaymentChart() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                if (dailyEPaymentBarChart) dailyEPaymentBarChart.destroy();
                ...
                if (dailyEPaymentLineChart) dailyEPaymentLineChart.destroy();
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'parkee', 'cash'].map((p, i) => ({
                    no: i + 1,
                    payment: p.charAt(0).toUpperCase() + p.slice(1).replace('tapcash', 'Tap Cash'),
                    last_week: formatRupiah(lastWeek[`${p}payment`]),
                    this_week: formatRupiah(thisWeek[`${p}payment`])
                }));
                ...
                const datasets = paymentTypes.map((p, i) => ({
                    label: p.charAt(0).toUpperCase() + p.slice(1).replace('tapcash', 'Tap Cash'),
                    data: thisWeekChart.map(item => item[`${p}payment`]),
                    backgroundColor: colors[i],
                    borderColor: colors[i],
                    hidden: i > 0,
                    datalabels: { anchor: 'end', align: 'end' }
                ...
                if (weeklyEPaymentBarChart) weeklyEPaymentBarChart.destroy();
                const ctxBar = document.getElementById('weeklyE-PaymentBar')?.getContext('2d');
                if (ctxBar) weeklyEPaymentBarChart = new Chart(ctxBar, { type: 'bar', data: { labels, datasets }, options: commonOptions, plugins: [ChartDataLabels] });

                if (weeklyEPaymentLineChart) weeklyEPaymentLineChart.destroy();
                const ctxLine = document.getElementById('weeklyE-PaymentLine')?.getContext('2d');
                if (ctxLine) weeklyEPaymentLineChart = new Chart(ctxLine, { type: 'line', data: { labels, datasets: datasets.map(ds => ({...ds, fill: false})) }, options: commonOptions, plugins: [ChartDataLabels] });
            }
        });
    }
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                const rows = ['emoney', 'flazz', 'brizzi', 'tapcash', 'parkee', 'cash'].map((p, i) => ({
                    no: i + 1,
                    payment: p.charAt(0).toUpperCase() + p.slice(1).replace('tapcash', 'Tap Cash'),
                    last_month: formatRupiah(lastMonth[`${p}payment`]),
                    this_month: formatRupiah(thisMonth[`${p}payment`])
                }));
                ...
                const datasets = paymentTypes.map((p, i) => ({
                    label: p.charAt(0).toUpperCase() + p.slice(1).replace('tapcash', 'Tap Cash'),
                    data: Object.values(thisMonthChart).map(item => item[`${p}payment`]),
                    backgroundColor: colors[i],
                    borderColor: colors[i],
                    hidden: i > 0,
                    datalabels: { anchor: 'end', align: 'end' }
                ...
                if (monthlyEPaymentBarChart) monthlyEPaymentBarChart.destroy();
                const ctxBar = document.getElementById('monthlyE-PaymentBar')?.getContext('2d');
                if (ctxBar) monthlyEPaymentBarChart = new Chart(ctxBar, { type: 'bar', data: { labels, datasets }, options: commonOptions, plugins: [ChartDataLabels] });

                if (monthlyEPaymentLineChart) monthlyEPaymentLineChart.destroy();
                const ctxLine = document.getElementById('monthlyE-PaymentLine')?.getContext('2d');
                if (ctxLine) monthlyEPaymentLineChart = new Chart(ctxLine, { type: 'line', data: { labels, datasets: datasets.map(ds => ({...ds, fill: false})) }, options: commonOptions, plugins: [ChartDataLabels] });
            }
        });
    }
    ...
    setInterval(loadAllDashboardData, 15000); // Refresh every 30 seconds
});
